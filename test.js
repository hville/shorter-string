...
import * as charset from './src/charset.js'

for (const set in charset) console.log(set, charset[set].length)

function test(lbl, txt, chr, dic) {
	const comp = encode(txt, chr, dic),
				back = decode(comp, chr, dic)
...
test('javascript',
`import {enc, dec, HASH} from './index.js'

function test(lbl, txt, chr=HASH, dic) {
...
const s = new Set()
for(const c of charset.MTF) {
	if (!s.has(c)) s.add(c)
	else console.log('!',c,c.charCodeAt(0),charset.MTF.indexOf(c))
}
